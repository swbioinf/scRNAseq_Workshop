...
```{r}
library(here)
seurat_object <- readRDS(here("data", "kang2018.rds"))
...
NB: A more detailed explanation of this format is here [biostats squid Seurat Objects explained]( https://biostatsquid.com/seurat-objects-explained/)
...
But some analyses need to be run on everything combined, and you'll need to run the 'JoinLayers' function to merge them back. Helpfully, you'll get an error when this is necessary: Like this one from _FindMarkers()_
...
You can undo that with the split function

```{r eval=FALSE}
seurat_object <- split(seurat_object, seurat_object$sample)
```

These commands for working with layers were introduced with Seurat 5, and described [here](https://satijalab.org/seurat/articles/seurat5_essential_commands.html)
