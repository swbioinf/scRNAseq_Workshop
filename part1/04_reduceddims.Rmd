...
1) Identify ~2000 variable genes - these are likely to be informative for cell types
2) Scale data - consider genes equally
3) Run PCA - Summarise ~2000 variable genes to ~20 dimensions 
4) Run UMAP - Summarise ~10-20 PCA dimensions to just 2 that we can see.
...
The procedure in Seurat is described in detail [here](https://doi.org/10.1016/j.cell.2019.05.031), and improves on previous versions by directly modelling the mean-variance relationship inherent in single-cell data, and is implemented in the `FindVariableFeatures()` function. By default, Seurat returns 2,000 features per dataset. These will be used in downstream analysis, like PCA.
...
```{r}
# Plotting gene expression on UMAP using featurePlot()
FeaturePlot(seurat_object, 'IFI6')
```
...
save_path <- here("data", "seurat_object_tutorial_saved.rds")
saveRDS(seurat_object, file = save_path)
```
