...
Use `names(so@meta.data)` to identify where this information is stored in the Seurat object.
...
Use the `table(so$<name>)` function to see the number of cells that belong to each metadata category. Replace `<name>` with the relevant metadata names (e.g. `ind`, `stim`)
