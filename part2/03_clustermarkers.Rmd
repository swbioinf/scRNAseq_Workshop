...
```{r}
c3 <- c("MS4A1", "CD79A", "IRF8", "CD83")
VlnPlot(seurat_object, features = c3)
# you can plot raw counts as well
VlnPlot(seurat_object, features = c3, layer = 'counts', log = TRUE)
```

```{r markerplots}
FeaturePlot(seurat_object, features = c3, reduction = "umap_harmony")

c2 <- c("GZMA", "GNLY", "PRF1", "CLIC3")
FeaturePlot(seurat_object, features = c2, reduction = "umap_harmony")
```
...
```{r ridgeplots}
RidgePlot(seurat_object, features = c2)
RidgePlot(seurat_object, features = c3)
```
...
```{r}
DotPlot(seurat_object, features = c(c3, c2))
```
...
#TODO: Also important to show plots of individual genes!
