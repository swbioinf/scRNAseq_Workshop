...
# Match the order of the matrix with the annotation. It did already match in htis case, but a mismatch would cause silent swapping of the data!
...
This code will build anothecounts matrix - but this one with a 'sample' for every combination of sample + celltype.
...
# Match the order of the matrix with the annotation. It did already match in htis case, but a mismatch would cause silent swapping of the data!
#pseudobulk_matrix <- pseudobulk_matrix[,pseudobulk_annotation_table$sample_celltype_name]
pseudobulk_matrix.allcelltypes[1:5,1:4]
```



This code builds a quick TMM normalised matrix. The limma-voom method we used for differential expression worked directly with counts data - but we need something normalised for examining expression!
It is customary to log2 transform expresison data - 

```{r}
dge <- DGEList(pseudobulk_matrix.allcelltypes) 
dge <- calcNormFactors(dge) 
norm_matrix <- cpm(dge)                 # Get normalised matrix according to normalisation factors.
...
1) Look at the expression of some other genes (differntially expressed or not). Are they globally changing or just in that celltype?  Share an interesting example.
2) [If time permits] Try plotting the raw counts - what do you notice?
