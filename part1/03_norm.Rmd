...
Right now, we have counts stored in the _counts_ layer of our _RNA_ assay.

```{r}
seurat_object
GetAssayData(seurat_object, assay='RNA', layer='counts')[60:70,1:10]
```
...
```
For clarity, in this previous line of code (and in future commands), we provide the default values for certain parameters in the function call. However, this isn't required and the same behaviour can be achieved with:
...
After normalisation, we have normalised data stored in the _data_ layer of our _RNA_ assay.

```{r}
seurat_object
GetAssayData(seurat_object, assay='RNA', layer='data')[60:70,1:10]
```
