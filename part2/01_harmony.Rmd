...
First we will inspect how our cells are clustering by inspecting how they group my the different annotations available.
...
EXERCISE: Use `DimPlot(seurat_object, reduction="umap", group.by=<metadata>)` to see how they group by control vs. stimulation condition. 
...
ANSWER IN SLACK: Compare the UMAP and UMAP Harmony plots. Describe the differences in how cells are clustering between the two. Have batch effects been corrected? 
...
```{r eval=F}
#qs2::qs_save(seurat_object, "../data/seurat_object_preprocessed.harmony.qs2")
```
