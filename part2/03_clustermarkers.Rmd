...
> **Cluster markers vs. canonical markers:** These are not the same thing. `FindMarkers` and `FindAllMarkers` identify genes that are statistically differentially expressed in a cluster compared to others: they tell you what makes a cluster *different*. A canonical marker is a gene whose expression in a specific cell type is well established in the literature: it tells you *what a cluster is*. The top DEGs from `FindAllMarkers` are a starting point for annotation, not an answer on their own. The top-down annotation section below shows how to go from one to the other.
...
We use `only.pos = TRUE` to return only positive markers — genes that are more highly expressed in the cluster than in others. These are the most useful for cell type identification: a gene being *switched off* in a cluster is harder to interpret than one being *switched on*.

```{r}
# find positive markers of cluster 1
cluster1.markers <- FindMarkers(seurat_object, ident.1 = 1, min.pct = 0.25, only.pos = TRUE)
head(cluster1.markers, n = 5)
```

```{r}
# find markers distinguishing cluster 1 from specific neighbouring clusters 2, 5, 6
cluster1.specific.markers <- FindMarkers(seurat_object, ident.1 = 1, ident.2 = c(2, 5, 6), min.pct = 0.25)
head(cluster1.specific.markers, n = 5)
```
...
```{r}
cluster3_markers <- seurat_object.markers %>%
  filter(cluster == 3) %>%
...
DoHeatmap(subset(seurat_object, downsample = 100), features = top10$gene) + NoLegend()
```
...
Note: IL7R also shows signal in cluster 6. IL7R is a T-lineage marker rather than CD4-specific, so this doesn't contradict a CD8 T cell call - it's consistent with cluster 6 being T cells.

**Cluster 7** shows LYZ (a broad myeloid marker) alongside CST3 and FCER1A, which are more specific for dendritic cells. Dendritic cells are a small population - look for a compact region on the UMAP that lights up for both CST3 and FCER1A but is distinct from the monocyte clusters. The absence of IL7R signal here reinforces that cluster 7 is myeloid rather than T-lineage:
