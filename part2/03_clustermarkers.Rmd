...
TODO: Mention automated ways to do this

## Top-down annotation with a marker table

TODO: Reframe so that the top DEGs may not always be sufficient, given reasons such as the top n genes may not be indicative of a cell type, that clustering is unsupervised and could be misleading. Therefore, it is important to take a "top-down" approach to label cells agafinst known markers from the literature.

For a well-characterised tissue, the most efficient way to do this is to start from a curated list of canonical markers: genes whose expression patterns are already established for the cell types we expect.
...
TODO: We won't do this for time, and dataset and question dependent
...
DISCUSS: How would you label cluster 4, and does the right answer change depending on the biological question being asked?
...
new.cluster.ids <- c(
  ...
  "5" = "",    # PF4, PPBP
  "6" = "",    # CD8A, CD8B (see FeaturePlot above)
  "7" = ""     # CST3, FCER1A (see FeaturePlot above)
)
```

TODO: Go to the catchup_script.R and run the code for this if required, that is ok.
...
We'll use it here to verify tthe identity of cluster 2.
...
TODO: In fact, this suggests that cluster 2 is too big of a cluster and could benefit from clustering at a higher resolution, scrnaseq analysis, especially annotation should be treated iteratively
